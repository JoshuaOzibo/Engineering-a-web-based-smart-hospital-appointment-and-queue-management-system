...
function NotificationsPage() {
  ...
  const { data: apptData, isLoading: apptLoading } = useQuery({
    ...
    queryFn: () =>
      isDoctor
        ? appointmentApi.getDoctorAppointments(matchedDoctor!._id)
  ...
  return (
    <AppLayout
      title="Notifications"
      subtitle="Reminders, appointment updates and messages from your care team."
    >
            ...
            <div className="flex flex-col items-center py-16 text-center gap-4 px-6">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground mt-1">
                  Appointment reminders and queue alerts will appear here.
                </div>
              </div>
              <Link
                to="/login"
                className="h-10 px-5 rounded-xl bg-primary text-primary-foreground text-sm font-medium inline-flex items-center gap-2"
              >
            ...
            <ul className="divide-y divide-border">
              {allItems.map((n, i) => (
                <li
                  key={i}
                  className={cn(
                    "p-5 flex gap-4 hover:bg-muted/30 transition-colors",
                    i < liveItems.length && "bg-primary/5",
                  )}
                >
                  <div
                    className={cn(
        ...
        <aside className="space-y-6">
          ...
          <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
            ...
            <div className="mt-4 rounded-xl bg-surface p-4 text-sm">
              ...
              <p className="mt-2">
                Hi {user?.name ?? "Patient"}, you have an upcoming appointment. Log in to Mediqueue
                to view details. Reply STOP to opt out.
              </p>
            </div>
          </div>
        </aside>
