...
function setAdminSession(s: AdminSession) {
...
const waitData = Array.from({ length: 12 }, (_, i) => ({
  h: `${8 + i}h`,
  w: 8 + Math.round(Math.sin(i / 2) * 4 + (i > 4 && i < 9 ? 6 : 0)),
}));
...
function OverviewTab({ onViewAppointments }: { onViewAppointments?: () => void }) {
  ...
  const kpis = [
    {
      k: "Registered patients",
      v: data?.usersRegistered?.length ?? 0,
      icon: Users,
      color: "bg-blue-500/10 text-blue-500",
    },
    {
      k: "Total appointments",
      v: data?.totalAppointments ?? 0,
      icon: CalendarCheck,
      color: "bg-primary/10 text-primary",
    },
    {
      k: "Upcoming appointments",
      v: data?.totalPendingAppointments ?? 0,
      icon: Clock,
      color: "bg-warning/20 text-warning-foreground",
    },
    {
      k: "Approved doctors",
      v: data?.docApproved?.length ?? 0,
      icon: Stethoscope,
    ...
    <div className="space-y-6">
        ...
        {kpis.map(({ k, v, icon: Icon, color }) => (
          <div key={k} className="rounded-2xl border border-border bg-card p-5 shadow-soft">
            <div className="flex items-center justify-between">
              ...
              <div className={cn("size-9 rounded-lg grid place-items-center", color)}>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
          ...
          <div className="h-60">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={deptData.length ? deptData : [{ d: "—", v: 0 }]}
                layout="vertical"
                margin={{ left: 0 }}
              >
                <CartesianGrid
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
        <div className="text-sm font-semibold mb-4 flex items-center justify-between">
          ...
          <span className="text-xs text-muted-foreground">
            {data?.appPending?.length ?? 0} awaiting approval
          </span>
        </div>
      ...
      {(data?.docPending?.length ?? 0) > 0 && (
        <div className="flex items-center gap-3 rounded-xl bg-warning/10 border border-warning/20 text-warning-foreground px-5 py-4 text-sm">
          ...
          <span>
            <strong>{data!.docPending.length}</strong> doctor
            {data!.docPending.length > 1 ? "s" : ""} awaiting approval. Go to the{" "}
...
function AppointmentsTab() {
  ...
  const shown =
    filter === "all" ? all : all.filter((a) => (filter === "pending" ? !a.status : a.status));
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Appointment rejected and removed.");
      qc.invalidateQueries({ queryKey: ["admin-appointments"] });
      qc.invalidateQueries({ queryKey: ["admin-dashboard"] });
    },
    onError: (e: Error) => toast.error(e.message),
                    ...
                    {!a.status && (
                      <div className="flex items-center gap-2">
                        ...
                        <ActionBtn
                          ...
                          onClick={() => {
                            if (window.confirm("Reject and delete this appointment?"))
                              rejectMutation.mutate(a._id);
                          }}
                        />
                      </div>
                    )}
    ...
    <div className="space-y-4">
      ...
      {pending.length > 0 && (
        <div className="flex items-center gap-3 rounded-xl bg-warning/10 border border-warning/20 text-warning-foreground px-5 py-3 text-sm">
          ...
          <span>
            <strong>{pending.length}</strong> pending application{pending.length > 1 ? "s" : ""}{" "}
            require your review.
          </span>
        </div>
      )}
      ...
      <div className="rounded-2xl border border-border bg-card shadow-soft overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="text-xs text-muted-foreground bg-surface">
              <tr>
                {["Doctor", "Qualifications", "Experience", "City", "Status", "Actions"].map(
                  (h) => (
                    <th key={h} className="text-left font-medium px-5 py-3">
                      {h}
                    </th>
                        ...
                        <div className="flex items-center gap-2">
                          <ActionBtn
                            ...
                            loading={
                              ...
                              (statusMutation.variables as { id: string }).id === d._id
                            }
                          ...
                          <ActionBtn
                            ...
                            onClick={() => {
                              if (window.confirm(`Reject Dr. ${d.doctorName}'s application?`))
                                statusMutation.mutate({ id: d._id, status: false });
                            }}
                          />
                        </div>
                      )}
              ...
              Array.from({ length: 5 }).map((_, i) => (
                <tr key={i} className="border-t border-border">
                  {[80, 140, 240].map((w, j) => (
                    <td key={j} className="px-5 py-3">
                      <div
                        className={`h-4 rounded bg-muted animate-pulse w-${w <= 80 ? "20" : w <= 140 ? "36" : "60"}`}
                      />
                    </td>
                  ))}
                </tr>
...
function ActionBtn({
  ...
  return (
    <button
      ...
      disabled={loading}
      className={cn(
        "inline-flex items-center gap-1 h-7 px-2.5 rounded-lg text-xs font-medium border disabled:opacity-50",
        tone === "success"
